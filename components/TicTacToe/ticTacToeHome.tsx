      ...
      <div className="h-screen bg-gradient-to-br from-purple-600 via-pink-500 to-orange-400 flex items-center justify-center p-4 relative overflow-hidden">
        <div className="absolute inset-0 opacity-20">
          ...
          <div className="absolute right-0 top-1/2 -translate-y-1/2 w-1/2 h-full flex flex-wrap content-start">
            <div className="text-white text-[30rem] font-bold p-4 left-72 absolute">
              ⭕
            </div>
          </div>
        </div>
    ...
    <div className="h-screen bg-gradient-to-br from-blue-600 via-purple-500 to-pink-500 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="backdrop-blur-xl bg-white/10 rounded-3xl p-4 shadow-2xl border border-white/20 max-w-3xl w-full relative z-10 flex gap-4">
        <div className="flex-1">
          <div className="flex justify-between items-center mb-4">
            <button
              onClick={backToMenu}
              className="px-3 py-1.5 bg-white/20 hover:bg-white/30 backdrop-blur-lg rounded-lg text-white font-medium transition-all duration-300 border border-white/30 text-sm"
            >
              ← Menu
            </button>
          ...
          <div className="grid grid-cols-3 gap-2 mb-4 relative">
            {board.map((cell, index) => (
              <button
                ...
                className={`aspect-square bg-white/20 backdrop-blur-lg rounded-xl border-2 border-white/30 text-4xl font-bold text-white transition-all duration-300 hover:bg-white/30 hover:scale-105 active:scale-95 disabled:cursor-not-allowed shadow-lg ${
                  winningLine.includes(index)
                    ? "bg-green-400/40 border-green-300/50 animate-pulse"
                    : ""
                }`}
              >
                {cell === "X" ? "❌" : cell === "O" ? "⭕" : ""}
          ...
          <div className="text-center">
            ...
            <button
              onClick={resetGame}
              className="px-5 py-2 bg-white/20 hover:bg-white/30 backdrop-blur-lg rounded-xl text-white font-semibold text-sm transition-all duration-300 border border-white/30 hover:scale-105 active:scale-95"
            >
              New Game
            </button>
          </div>
        ...
        <div className="w-48 backdrop-blur-lg bg-white/5 p-3 rounded-2xl border border-white/20 flex flex-col gap-3">
      ...
      {showResult && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-300">
          <div className="backdrop-blur-xl bg-white/20 rounded-3xl p-8 shadow-2xl border-2 border-white/30 max-w-md w-full animate-in zoom-in duration-300">
            <div className="text-center">
              <div className="text-8xl mb-4 animate-bounce">
                {winner === "X" ? "❌" : winner === "O" ? "⭕" : "🤝"}
              </div>
              <h2 className="text-4xl font-bold text-white mb-4 drop-shadow-lg animate-in slide-in-from-top duration-500">
                {winner === "Draw" ? "It's a Draw!" : `${winner} Wins!`}
              </h2>
              <p className="text-white/90 text-xl mb-8 animate-in slide-in-from-bottom duration-500">
